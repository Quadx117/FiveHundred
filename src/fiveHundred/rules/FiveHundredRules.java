...
public class FiveHundredRules {
	 ...
	 * during this trick.
	 * 
	 * @param cardsOnTable The HashMap containing the cards currently played on the table.
	 * @param firstCardPlayed An index into the map of cards that represents the first card played on the table.
	...
	public static int trickWinner(HashMap<Integer, Card> cardsOnTable, int firstCardPlayed) {
		...
		for (int offset = 1; offset < numberOfPlayers; ++offset) {
			if ((cardsOnTable.get((firstCardPlayed + offset) % numberOfPlayers).getSuit() == Card.JOKER	&&
					cardsOnTable.get((firstCardPlayed + offset) % numberOfPlayers).getValue() > cardsOnTable.get(trickWinner).getValue())
				|| (cardsOnTable.get((firstCardPlayed + offset) % numberOfPlayers).getSuit() == cardsOnTable.get(firstCardPlayed).getSuit()
					&& cardsOnTable.get((firstCardPlayed + offset) % numberOfPlayers).getValue() > cardsOnTable.get(trickWinner).getValue()))
			{
		...
		/*if (cardsOnTable.get((firstCardPlayed + 1) % numberOfPlayers).getSuit() == Card.JOKER
				&& cardsOnTable.get((firstCardPlayed + 1) % numberOfPlayers).getValue() > cardsOnTable.get(trickWinner)
						.getValue()
				|| cardsOnTable.get((firstCardPlayed + 1) % numberOfPlayers).getSuit() == cardsOnTable.get(
		...
		if (cardsOnTable.get((firstCardPlayed + 2) % numberOfPlayers).getSuit() == Card.JOKER
				&& cardsOnTable.get((firstCardPlayed + 2) % numberOfPlayers).getValue() > cardsOnTable.get(trickWinner)
						.getValue()
				|| cardsOnTable.get((firstCardPlayed + 2) % numberOfPlayers).getSuit() == cardsOnTable.get(
						firstCardPlayed).getSuit()
				&& cardsOnTable.get((firstCardPlayed + 2) % numberOfPlayers).getValue() > cardsOnTable.get(trickWinner)
						.getValue()) {
